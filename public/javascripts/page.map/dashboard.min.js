...
function initMarkerIcon(marker){
  ...
  if(marker.isAdd){
    ...
    iconImageFocus = '/images/marker-pin-passive-active.png'
  }
...
function initMapData(selectedDate){
  ...
  alert(`/api/maps/listpin/${$('#hdn-mapid').val().trim()}`)

  $.post({
    url: `/api/maps/listpin/${$('#hdn-mapid').val().trim()}`,
    data: {
      ...
      if(data.length > 0){
        ...
        if(data.length > 1){
          for(var idx = 1; idx < data.length; idx++){
            var newMarker = {
              "name": data[idx].pinDropName,
              "type": 1,
              "pos": {
                "lat": parseFloat(data[idx].position.lat),
                "lng": parseFloat(data[idx].position.lng)
              },
              "fieldList": [
                {
                  "type": data[idx].listField[0].fieldType,
                  "name": data[idx].listField[0].fieldName,
                  "condition": data[idx].listField[0].targetComparison,
                  "target": data[idx].listField[0].targetValue,
                  "value": data[idx].listField[0].value || null
                }
              ],
              "supervisor": data[idx].supervisor.userEmail,
              "isAdd": false,
              "isPass": data[idx].listField[0].isPass || false
            }
  ...
  markerDelete.unbind().on('click', function(){
    ...
    $.ajax({
      ...
      data: {
        ...
        lat:markPinDrops[selectedIndex].position.lat() ,
        lng: markPinDrops[selectedIndex].position.lng()
      },
  ...
  markerSave.unbind().on('click', function(){
    ...
    if(markPinDrops[selectedIndex].isAdd){
      ...
      $.ajax({
        ...
        type: 'POST',
        data: {
          pinDropName: markPinDrops[selectedIndex].title,
          totalSales: markPinDrops[selectedIndex].fieldList[0].target,
          salesCond: markPinDrops[selectedIndex].fieldList[0].condition,
          supervisor: markPinDrops[selectedIndex].supervisor,
          fieldName: markPinDrops[selectedIndex].fieldList[0].name,
          userID: $('#hdn-userid').val().trim(),
